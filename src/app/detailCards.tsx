...
export default function DetailCards(){
    const [backDetail, setBackDetail] = useState<number>(0);
    const [frontDetail, setFrontDetail] = useState<number>(0);

    const handleBackDeatil = () => {
        setBackDetail(backDetail + 1);
    };
    const handleFrontDeatil = () => {
        setFrontDetail(frontDetail + 1);
    };

    return (
        <div>
            <div className="button-container">
                <div className="back-end-button" onClick={handleBackDeatil}>
                    view more
                </div>
                <div className="front-end-button" onClick={handleFrontDeatil}>
                    view more
                </div>
            </div>
            <div className="detail-container">
                ...
                <div className="front-detail-card">
                    <div className="bf-detail">
                        ...
                        <div className="detail-detail">
                            <h4><span>구조체</span></h4>
                            <p><span>포인터</span>는 <span>특정 변수의 주소값을 저장하는 변수</span>로, 프로그램을 세부적으로 관리하거나 메모리에 직접적으로 접근하기 위해서 필요한 기술입니다. 프로젝트에서 <span>배열의 반환값</span>을 포인터로 받으면서 기존에는 불가능한 배열의 반환을 가능하게 해주었습니다.</p>
                        </div>
                    </div>
                    <div className="bf-detail">
                        <h4>TypeScript</h4>
